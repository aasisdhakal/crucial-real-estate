(function ($) {
    ...
    $(window).on('load', function () {
        ...
        if (jQuery().validate && jQuery().ajaxSubmit) {
            var ereGetContactFormWidgets = $('.cre_widget_contact_form');
            if (ereGetContactFormWidgets.length) {
                $.each(ereGetContactFormWidgets, function (i, widget) {
                    var id = $(widget).attr("id");
                    ereWidgetContactForm('#' + id + ' .cre-contact-form');
                });
            }
        }

        /**
         * Crucial Real Estate Social Login
         */
        $('.rsl-provider').on('click', function(e){
            ...
            $.ajax({
                type: 'POST',
                url: cre_social_login_data.ajax_url,
                dataType: 'json',
                data: {
                    'action' : 'cre_'+ $provider +'_oauth_url'
                },
                beforeSend: function( ) {
    ...
    $(document).on('ready',function () {
        $('body').on('click', '.cre_pagination_wrapper a', function (e) {
            e.preventDefault();

            var thisParent = $(this).parents('.cre_ele_property_ajax_target');
            var id = $(thisParent).attr('id');
            var thisLoader = $(thisParent).find('.rhea_svg_loader');
            var thisInner = $(thisParent).find('.home-properties-section-inner-target');
            var pageNav = $(thisParent).find('.cre_pagination_wrapper a');
            var thisLink = $(this);
