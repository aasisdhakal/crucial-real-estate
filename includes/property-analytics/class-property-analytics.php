 ...
 */
require_once CRE_PLUGIN_DIR . 'includes/property-analytics/class-operating-system-browser.php';
...
class Property_Analytics
{
	...
	public function insert_property_view()
	{
		...
		$wpdb->insert(
			"{$wpdb->prefix}aarambha_property_analytics",
			array(
				'PID'       => $property_id,
 ...
 */
function cre_initialize_property_analytics()
{
	new Property_Analytics();
}
add_action('wp', 'cre_initialize_property_analytics');


if (!function_exists('cre_create_property_analytics_table')) {
	/**
	 * Create required database table.
	 */
	function cre_create_property_analytics_table()
	{
		...
		$sql_query = "CREATE TABLE {$wpdb->prefix}aarambha_property_analytics (
	...
	add_action('admin_head', 'cre_create_property_analytics_table');
}
