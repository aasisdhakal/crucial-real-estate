...
/**
 * This file contains Property_Analytics_View class and 'aarambha_get_property_views' Ajax handler
	...
	public function get_views($type = 'all', $date = '')
	{
		...
		switch ($type) {
			case 'property_unique_views':
				...
				$sql_query = "SELECT COUNT(DISTINCT IP) FROM {$wpdb->prefix}aarambha_property_analytics WHERE PID = {$this->property_id} ";
			...
			case 'property_today_views':
				...
				$sql_query = "SELECT COUNT(PID) FROM {$wpdb->prefix}aarambha_property_analytics WHERE PID = {$this->property_id} AND TimeStamp LIKE CONCAT('$date','%')";
				$result    = $wpdb->get_var($sql_query); // phpcs:ignore
				break;
			case 'property_total_views':
				// Property total views.
				$sql_query = "SELECT COUNT(PID) FROM {$wpdb->prefix}aarambha_property_analytics WHERE PID = {$this->property_id}";
				$result    = $wpdb->get_var($sql_query); // phpcs:ignore
				break;
			case 'property_views_history':
				// Property total views.
				$sql_query = "SELECT * FROM {$wpdb->prefix}aarambha_property_analytics WHERE PID = {$this->property_id}";
				$result    = $wpdb->get_results($sql_query); // phpcs:ignore
				break;
			default:
				// Properties total views.
				$sql_query = "SELECT COUNT(PID) FROM {$wpdb->prefix}aarambha_property_analytics";
				$result    = $wpdb->get_var($sql_query); // phpcs:ignore
		}
...
if (!function_exists('aarambha_get_property_views')) {
	/**
	 * Ajax request handler to fulfil requested property
	 * views result based on given property ID.
	 */
	function aarambha_get_property_views()
	{

		if (empty($_REQUEST['nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_REQUEST['nonce'])), 'cre-property-analytics')) {
		...
		if (isset($_POST['property_id']) && !empty($_POST['property_id'])) {
			...
			$total_views = array();
			$num_of_days = get_option('aarambha_property_analytics_time_period', 14);
	...
	add_action('wp_ajax_aarambha_property_views', 'aarambha_get_property_views');
	add_action('wp_ajax_nopriv_aarambha_property_views', 'aarambha_get_property_views');
}
