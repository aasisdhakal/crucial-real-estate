	...
	class Agents_List_Widget extends WP_Widget
	{
		...
		function __construct()
		{
			$widget_ops = array(
				...
				'description'                 => esc_html__('This widget displays agents list.', 'crucial-real-estate'),
				'customize_selective_refresh' => true,
			);
			parent::__construct('Agents_List_Widget', esc_html__('Crucial Real Estate - Agents', 'crucial-real-estate'), $widget_ops);
		}
		...
		function widget($args, $instance)
		{
			...
			$title = !empty($instance['title']) ? $instance['title'] : esc_html__('Agents', 'crucial-real-estate');
			...
			$agents_query = new WP_Query(apply_filters('cre_agents_widget', $agent_args));
			...
			?>
				<div class="agents-list-widget">
					<article class="agent-list-item">
						<?php echo '<h4>' . esc_html__('No Agent Found!', 'crucial-real-estate') . '</h4>'; ?>
					</article>
				</div>
			<?php
			endif;
		...
		function form($instance)
		{
			$instance = wp_parse_args(
				(array) $instance,
				array(
					'title' => esc_html__('Agents', 'crucial-real-estate'),
					'count' => 3
				)
			);
			...
			<p>
				<label for="<?php echo esc_attr($this->get_field_id('count')); ?>"><?php esc_html_e('Number of Agents', 'crucial-real-estate'); ?></label>
