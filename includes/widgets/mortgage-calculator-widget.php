...
/**
 * Crucial Real Estate - Mortgage Calculator Widget.
 *
 * @since      3.11.1
 * @package    easy-realestate
 * @subpackage crucial-real-estate/widgets
 */

if (!class_exists('Crucial_Real_Estate_Mortgage_Calculator_Widget')) {
	/**
	 * Crucial Real Estate - Mortgage Calculator Widget Class
	 *
	 * @since 3.11.1
	 */
	class Crucial_Real_Estate_Mortgage_Calculator_Widget extends WP_Widget
	{
		...
		{
			parent::__construct(
				'crucial_real_estate_mortgage_calculator', // Base ID of the widget.
				esc_html__('Crucial Real Estate - Mortgage Calculator', 'crucial-real-estate'), // Widget name will appear in UI.
				array('description' => esc_html__('This widget display Mortgage Calculator.', 'crucial-real-estate')) // Widget description.
			);
		}
		...
		public function widget($args, $instance)
		{
			...
			$property_rent_status = get_option('aarambha_mortgage_calculator_statuses'); // Get property enabled statuses for the mortgage calculator.
...
?>
				<div class="rh_property__mc_wrap">
					<div class="rh_property__mc clearfix">
						...
						$mc_default['terms']       = array_map('intval', explode(',', get_option('aarambha_mc_terms', '30,20,15,10,5')));
						$mc_default['term']        = intval(get_option('aarambha_mc_term_default', '10'));
						$mc_default['interest']    = floatval(get_option('aarambha_mc_interest_default', '3.5'));
						$mc_default['downpayment'] = floatval(get_option('aarambha_mc_downpayment_default', '20'));
						$mc_default['tax']         = intval(get_post_meta(get_the_ID(), 'aarambha_property_tax', true));
						$mc_default['fee']         = intval(get_post_meta(get_the_ID(), 'aarambha_additional_fee', true));
						...
						if (empty($mc_default['price'])) {
							$mc_default['price'] = intval(get_option('aarambha_mc_price_default', '0'));
						...
						if (function_exists('crucial_real_estate_currency_switcher_enabled') && crucial_real_estate_currency_switcher_enabled()) {
							$base_currency    = crucial_real_estate_get_base_currency();
							$current_currency = crucial_real_estate_get_current_currency();

							$mc_default['price_converted'] = crucial_real_estate_convert_currency($mc_default['price'], $base_currency, $current_currency);
							$mc_default['tax_converted']   = crucial_real_estate_convert_currency($mc_default['tax'], $base_currency, $current_currency);
							$mc_default['fee_converted']   = crucial_real_estate_convert_currency($mc_default['fee'], $base_currency, $current_currency);

							$currencies_data   = crucial_real_estate_get_currencies_data();
						...
						// Fields labels.
						$term_label        = get_option('aarambha_mc_term_field_label', esc_html__('Term', 'crucial-real-estate'));
						$interest_label    = get_option('aarambha_mc_interest_field_label', esc_html__('Interest', 'crucial-real-estate'));
						$price_label       = get_option('aarambha_mc_price_field_label', esc_html__('Home Price', 'crucial-real-estate'));
						$downpayment_label = get_option('aarambha_mc_downpayment_field_label', esc_html__('Down Payment', 'crucial-real-estate'));
						$principle_label   = get_option('aarambha_mc_principle_field_label', esc_html__('Principle and Interest', 'crucial-real-estate'));
						$cost_prefix       = get_option('aarambha_mc_cost_prefix', esc_html__('per month', 'crucial-real-estate'));
						?>
						<!-- Calculator right side -->
						<?php $graph_type = get_option('aarambha_mc_graph_type', 'circle'); ?>
						<div class="mc_right_side <?php echo 'graph_' . esc_attr($graph_type); ?>">
							<?php
							$tax_field_display = get_option('aarambha_mc_first_field_display', 'true');
							$hoa_field_display = get_option('aarambha_mc_second_field_display', 'true');

							if ('true' !== $tax_field_display) {
								$mc_default['tax_converted'] = '0';
								$mc_default['tax']           = '0';
							} else {
								$tax_field_label = get_option('aarambha_mc_first_field_title', esc_html__('Property Taxes', 'crucial-real-estate'));
							}
							...
							if ('true' !== $hoa_field_display) {
								...
								$hoa_field_label = get_option('aarambha_mc_second_field_title', esc_html__('Additional Fee', 'crucial-real-estate'));
							}

							$graph_type = get_option('aarambha_mc_graph_type', 'circle');
							...
							?>
								...
								<div class="mc_term_interest"><span class="mc_term_value"><?php echo esc_html($mc_default['term']); ?></span> <?php esc_html_e('Years Fixed', 'crucial-real-estate'); ?>, <span class="mc_interest_value"><?php echo esc_attr($mc_default['interest']); ?></span><span>%</span> <?php echo esc_html($interest_label); ?></div>
							<?php
							} else {
							?>
							...
							<div class="mc_cost">
								<ul>
									...
									?>
										<li class="mc_cost_tax"><?php echo esc_html($tax_field_label); ?> <span><?php echo esc_html(cre_format_amount($mc_default['tax'])); ?></span></li>
									<?php
									}

									if (!empty($mc_default['fee'])) {
									?>
										<li class="mc_cost_hoa"><?php echo esc_html($hoa_field_label); ?> <span><?php echo esc_html(cre_format_amount($mc_default['fee'])); ?></span></li>
									<?php
									}
									?>
						...
						<div class="mc_left_side">
							...
							<div class="rh_mc_field">
								...
								<div class="rh_form__item">
									<select name="mc_term" class="mc_term aarambha_select_picker_trigger aarambha_bs_default_mod aarambha_bs_green show-tick">
										<?php
										foreach ($mc_default['terms'] as $mc_term) {
											echo '<option value="' . esc_attr($mc_term) . '" ' . selected($mc_default['term'], $mc_term, false) . '>' . esc_html($mc_term) . ' ' . esc_html__('Years Fixed', 'crucial-real-estate') . '</option>';
										}
										?>
									</select>
								</div>
		...
		public function form($instance)
		{
			if (isset($instance['title'])) {
				...
				$title = esc_html__('Mortgage Calculator', 'crucial-real-estate');
			}
			// Widget admin form.
			?>
			<p>
				<label for="<?php echo esc_html($this->get_field_id('title')); ?>"><?php esc_html_e('Title:', 'crucial-real-estate'); ?></label>
...
// TODO: redo it.
function aarambha_load_widget()
{
	register_widget('Crucial_Real_Estate_Mortgage_Calculator_Widget');
}
add_action('widgets_init', 'aarambha_load_widget');
