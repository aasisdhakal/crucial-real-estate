	...
	class Agent_Featured_Properties_Widget extends WP_Widget
	{
		...
		function __construct()
		{
			$widget_ops = array(
				...
				'description'                 => esc_html__('Important: This widget is only for the Agent Sidebar.', 'crucial-real-estate'),
				'customize_selective_refresh' => true,
			);
			parent::__construct('Agent_Featured_Properties_Widget', esc_html__('Crucial Real Estate - Agent Featured Properties', 'crucial-real-estate'), $widget_ops);
		}
		...
		function widget($args, $instance)
		{
			...
			$agent_featured_properties_query = new WP_Query(apply_filters('cre_agent_featured_properties_widget', $agent_featured_properties_args));
...
?>
						<ul class="featured-properties">
							...
							?>
								<li>
									...
									<p><?php
										cre_framework_excerpt(7);
										if (isset($view_property) && !empty($view_property)) {
										?><a href="<?php the_permalink(); ?>"><?php echo esc_html($view_property); ?></a><?php
																														} else {
																															?><a href="<?php the_permalink(); ?>"><?php esc_html_e('Read More', 'crucial-real-estate'); ?></a><?php
																																																							}
																																																								?></p><?php
					...
					?>
						<ul class="featured-properties">
							<?php
							echo '<li>';
							esc_html_e('No featured property found for this agent.', 'crucial-real-estate');
							echo '</li>';
							?>
						</ul>
						<?php
					...
					if ($agent_featured_properties_query->have_posts()) :
						...
						<div class="rh_alert-wrapper rh_alert__widget">
							<h4 class="no-results"><?php esc_html_e('No Featured Property Found For This Agent', 'crucial-real-estate'); ?></h4>
						</div>
			<?php
					endif;
				}
		...
		function form($instance)
		{
			...
			?>
			<p>
				<label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php _e('Widget Title', 'crucial-real-estate'); ?></label>
				<input class="widefat" id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" type="text" value="<?php echo esc_attr($title); ?>" />
			</p>
			<p>
				<label for="<?php echo esc_attr($this->get_field_id('sort_by')); ?>"><?php _e('Sort By:', 'crucial-real-estate'); ?></label>
				<select name="<?php echo esc_attr($this->get_field_name('sort_by')); ?>" id="<?php echo esc_attr($this->get_field_id('sort_by')); ?>" class="widefat">
					<option value="recent" <?php selected($sort_by, 'recent'); ?>><?php _e('Most Recent', 'crucial-real-estate'); ?></option>
					<option value="random" <?php selected($sort_by, 'random'); ?>><?php _e('Random', 'crucial-real-estate'); ?></option>
				</select>
			</p>
			<p>
				<label for="<?php echo esc_attr($this->get_field_id('count')); ?>"><?php _e('Number of Properties', 'crucial-real-estate'); ?></label>
