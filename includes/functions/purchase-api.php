...
if (!class_exists('CRE_Purchase_API')) {
	class CRE_Purchase_API
	{
		...
		public function __construct()
		{
			...
			if (self::status() && get_transient('aarambha_activation_thanks')) {
		...
		{
			return ('yes' == get_option('aarambha_Crucial Real Estate_registered', 'no'));
		}

		public function after_register_notice()
		{
?>
			<div class="notice notice-success Crucial Real Estate-registration is-dismissible">
				<p>Thanks for verifying Crucial Real Estate purchase!</p>
			</div>
			<?php
			delete_transient('aarambha_activation_thanks');
		}
		...
		public function register_license()
		{
			...
			if (isset($_POST['purchase_code'])) {
				...
				if ($verification) {
					...
					set_transient('aarambha_activation_thanks', true, 5);
					...
					aarambha_redirect($current_url);
				}
			}

			if (!get_transient('aarambha_activation_thanks')) {
				$attempt_notice = get_option('Crucial Real Estate_verification_attempt_notice');
				$notice_class   = ('true' === $attempt_notice) ? 'error' : 'info';
			?>
				<div class="notice notice-<?php echo esc_attr($notice_class); ?> Crucial Real Estate-registration">
					<?php
					if ('true' === $attempt_notice) {
					?>
						<h3>Crucial Real Estate License Purchase Verification Failed</h3>
					...
					?>
						<h3>Crucial Real Estate License Purchase Verification</h3>
					...
					<p>You can consult <a href="https://support.inspirythemes.com/knowledgebase/how-to-get-themeforest-item-purchase-code/" target="_blank">this knowledge base article</a> to learn how to get themeforest item purchase code,
						Otherwise you can buy <a href="https://themeforest.net/item/real-homes-wordpress-real-estate-theme/5373914" target="_blank">Crucial Real Estate fresh license</a> that comes with lifetime updates and 6 months extendable support.</p>
				</div>
<?php
			}
		}
		...
		public function verify_purchase_code($code)
		{
			...
			if (!is_wp_error($envato_purchases) && is_string($envato_purchases['body'])) {
				...
				if (isset($body_array['item_id']) && '5373914' == $body_array['item_id']) {
					...
					update_option('aarambha_Crucial Real Estate_registered', 'yes');
					delete_option('Crucial Real Estate_verification_attempt_notice');
					return true;
				} else {
