...
if (!function_exists('cre_get_property_types')) {
	 ...
	 * @return string
	 */
	function cre_get_property_types($property_post_id)
	{
...
if (!function_exists('cre_get_property_types_array')) :
	/**
	 * Return associative array of property type terms. Where slug is key and name is value.
	 * @return array
	 */
	function cre_get_property_types_array()
	{
		$types_array = array();
...
if (!function_exists('cre_get_property_features_array')) :
	/**
	 * Return associative array of property feature terms. Where slug is key and name is value.
	 * @return array
	 */
	function cre_get_property_features_array()
	{
...
if (!function_exists('cre_any_text')) :
	/**
	 * Return text string for word 'Any'
	 *
	 * @return string
	 */
	function cre_any_text()
	{
		$cre_any_text = get_option('aarambha_any_text');
		if ($cre_any_text) {
			return $cre_any_text;
		}
		return esc_html__('Any', 'crucial-real-estate');
	}
endif;


if (!function_exists('cre_get_terms_array')) {
	/**
	 ...
	 * @param $terms_array
	 */
	function cre_get_terms_array($tax_name, &$terms_array)
	{
		$tax_terms = get_terms(array(
			'taxonomy'   => $tax_name,
			'hide_empty' => false,
		));
		cre_add_term_children(0, $tax_terms, $terms_array);
	}
}


if (!function_exists('cre_is_search_page_configured')) :
	/**
	 * Check if search page settings are configured
	 */
	function cre_is_search_page_configured()
	{

		/* Check search page */
		$aarambha_search_page = get_option('aarambha_search_page');
		if (!empty($aarambha_search_page)) {
			return true;
		}
...
if (!function_exists('cre_add_term_children')) :
	/**
	 ...
	 * @param string $prefix
	 */
	function cre_add_term_children($parent_id, $tax_terms, &$terms_array, $prefix = '')
	{
		if (!empty($tax_terms) && !is_wp_error($tax_terms)) {
			foreach ($tax_terms as $term) {
				if ($term->parent == $parent_id) {
					$terms_array[$term->slug] = $prefix . $term->name;
					cre_add_term_children($term->term_id, $tax_terms, $terms_array, $prefix . '- ');
				}
			}
		}
	}
...
if (!function_exists('cre_skip_sticky_properties')) :
	/**
	 * Skip sticky properties
	 */
	function cre_skip_sticky_properties()
	{
		$skip_sticky = get_option('aarambha_listing_skip_sticky', false);
		if ($skip_sticky) {
			remove_filter('the_posts', 'aarambha_make_properties_stick_at_top', 10);
		}
	}
endif;

if (!function_exists('cre_skip_home_sticky_properties')) :
	/**
	 * Skip sticky properties
	 */
	function cre_skip_home_sticky_properties()
	{
		$skip_sticky = get_post_meta(get_the_ID(), 'aarambha_home_skip_sticky', true);
		if ($skip_sticky) {
			remove_filter('the_posts', 'aarambha_make_properties_stick_at_top', 10);
		}
	}
...
if (!function_exists('cre_get_figure_caption')) {
	/**
	 ...
	 * @return string
	 */
	function cre_get_figure_caption($post_id)
	{
		$status_terms = get_the_terms($post_id, "property-status");
		if (!empty($status_terms)) {
			$status_classes = '';
...
if (!function_exists('cre_display_figcaption')) {
	/**
	 * Display figure caption for given property's post id
	 *
	 * @param $post_id
	 */
	function cre_display_figcaption($post_id)
	{
		echo cre_get_figure_caption($post_id);
	}
}


if (!function_exists('cre_is_added_to_compare')) {
	/**
	 ...
	 * @return bool
	 */
	function cre_is_added_to_compare($property_id)
	{

		if ($property_id > 0) {
			/* check cookies for property id */
			if (isset($_COOKIE['aarambha_compare'])) {
				$aarambha_compare 	= unserialize($_COOKIE['aarambha_compare']);
				if (in_array($property_id, $aarambha_compare)) {
					return true;
				}
			}
		}
...
if (!function_exists('cre_additional_details_migration')) {
	/**
	 ...
	 */
	function cre_additional_details_migration($post_id)
	{
