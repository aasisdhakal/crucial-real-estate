...
if (!function_exists('cre_is_gdpr_enabled')) {
	...
	function cre_is_gdpr_enabled()
	{
		...
		if (intval(get_option('aarambha_gdpr', '0'))) {
			return true;
		}
...
if (!function_exists('cre_gdpr_agreement')) {
	...
	function cre_gdpr_agreement($args = array())
	{
		...
		if (cre_is_gdpr_enabled()) {
			...
			$html       = '';
			$gdpr_label = get_option('aarambha_gdpr_label', esc_html__('GDPR Agreement', 'crucial-real-estate'));
			$validation = get_option('aarambha_gdpr_validation_text', esc_html__('* Please accept GDPR agreement', 'crucial-real-estate'));
			$gdpr_text  = get_option('aarambha_gdpr_text', esc_html__('I consent to having this website store my submitted information so they can respond to my inquiry.', 'crucial-real-estate'));
